...
public class FileController {
    ...
    private static final int LOG_INTERVAL = 100; // Her 100 işlemde bir log
    ...
    // Kesikli loglama için yardımcı metod
    private void logIntermittently(String message, Object... args) {
        int count = operationCounter.incrementAndGet();
        if (count % LOG_INTERVAL == 0) {
            logger.info(message, args);
            // Sayaç çok büyümesin diye sıfırla
            if (count > LOG_INTERVAL * 10) {
                operationCounter.set(0);
            }
        }
    }

    // Kritik işlemler için detaylı loglama
    private void logCriticalOperation(String operation, String username, String details) {
        logger.info("Kritik İşlem - {} - Kullanıcı: {} - Detaylar: {}", operation, username, details);
    }
    ...
    public String home(Model model, 
        ...
        try {
            ...
            logIntermittently("Ana sayfa görüntülendi - Kullanıcı: {}, Sayfa: {}, Dosya Sayısı: {}", 
                username, page, filesPage.getNumberOfElements());
            ...
            logger.error("Ana sayfa hatası: {}", e.getMessage());
    ...
    public String handleHomeFileUpload(@RequestParam("file") MultipartFile file,
        ...
        try {
            ...
            logCriticalOperation("Dosya Yükleme", username, 
                String.format("Dosya: %s, Boyut: %d, Klasör: %s", 
                    file.getOriginalFilename(), 
                    file.getSize(),
                    folder != null ? folder.getName() : "Ana Klasör"));
    ...
    @GetMapping("/search")
    @ResponseBody
    public ResponseEntity<?> searchFilesAjax(@RequestParam("keyword") String keyword,
                                           @RequestParam(defaultValue = "0") int page,
                                           @RequestParam(defaultValue = "5") int size) {
        try {
            Authentication auth = SecurityContextHolder.getContext().getAuthentication();
            if (auth == null || !auth.isAuthenticated()) {
                logger.warn("Yetkisiz arama denemesi");
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body("Kullanıcı girişi gerekli");
            }

            String username = auth.getName();
            logger.info("Arama yapıldı - Kullanıcı: {}, Anahtar kelime: {}", username, keyword);
            
            if (keyword == null || keyword.trim().isEmpty()) {
                logger.warn("Boş arama anahtar kelimesi - Kullanıcı: {}", username);
                return ResponseEntity.badRequest()
                    .body("Arama anahtar kelimesi boş olamaz");
            }
            
            Page<FileModel> results = fileService.searchUserFilesByName(username, keyword.trim(), page, size);
            
            if (results.getTotalElements() > 0) {
                logger.info("Arama sonucu bulundu - Kullanıcı: {}, Anahtar kelime: {}, Sonuç sayısı: {}", 
                    username, keyword, results.getTotalElements());
            } else {
                logger.info("Arama sonucu bulunamadı - Kullanıcı: {}, Anahtar kelime: {}", 
                    username, keyword);
            }
            
            return ResponseEntity.ok(results);
        } catch (Exception e) {
            logger.error("Arama hatası - Kullanıcı: {}, Anahtar kelime: {}, Hata: {}", 
                SecurityContextHolder.getContext().getAuthentication().getName(), 
                keyword, e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body("Arama sırasında bir hata oluştu: " + e.getMessage());
        }
    }
